...
func ListWarehousesHandler(store *storage.SQLStore) ginpkg.HandlerFunc {
	return func(c *ginpkg.Context) {
		...
		biz := warehousebiz.NewListWarehousesBiz(store)
		warehouses, err := biz.ListWarehouses(c.Request.Context(), &filter, paging)
